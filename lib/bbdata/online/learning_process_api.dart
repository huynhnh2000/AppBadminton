...
class LearningProcessApi{
  ...
  Future<bool> updateProcess(MyLearningProcess learningProcess) async{
    try{
      ...
      final res = await http.put(
        Uri.parse("$baseUrl/${dotenv.env["LEARNINGPROCESS_URL"]}/${int.parse(learningProcess.id!)}"),
        headers: {"Authorization": "Bearer ${currentUser.key}", "Content-Type": "application/json"},
        body: jsonEncode(body)
      ).timeout(const Duration(seconds: 30));
  ...
  Future<MyLearningProcess?> getLearningProcess(String studentId, String? dateCreated) async{
    try{
      DateFormat formatted = DateFormat("yyyy-MM-dd");
      String dateNow = dateCreated?.split("T")[0]??formatted.format(DateTime.now());

      final res = await http.get(
        Uri.parse("$baseUrl/${dotenv.env["LEARNINGPROCESS_URL"]}/studentId=${int.parse(studentId)}&date=$dateNow"),
        headers: {"Authorization": "Bearer ${currentUser.key}", "Content-Type": "application/json"}
      ).timeout(const Duration(seconds: 30));
      ...
      if(res.statusCode==200){
        ...
        if(data.isNotEmpty){
          MyLearningProcess learningProcess = MyLearningProcess.fromJson(data[0]);
  ...
  Future<List<MyLearningProcess>> getAllListSortStudentId(String studentId) async{
    try{
      final res = await http.get(
        Uri.parse("$baseUrl/${dotenv.env["LEARNINGPROCESS_URL"]}"),
        headers: {"Authorization": "Bearer ${currentUser.key}", "Content-Type": "application/json"}
  ...
  Future<List<MyLearningProcess>> getListWithStudentId(String studentId) async{
    try{
      final res = await http.get(
        Uri.parse("$baseUrl/${dotenv.env["LEARNINGPROCESS_URL"]}/studentId/${int.parse(studentId)}"),
        headers: {"Authorization": "Bearer ${currentUser.key}", "Content-Type": "application/json"}
      ).timeout(const Duration(seconds: 30));
